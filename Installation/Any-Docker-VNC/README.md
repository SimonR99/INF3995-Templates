...
```bash
docker build -t teamx-ros-gz .
```

After creating the image, run the following command to start the server:
```bash
docker run --rm -p 5901:5901 -it teamx-ros-gz bash -c "TVNC_WM=xfce /opt/TurboVNC/bin/vncserver -securitytypes TLSNone,X509None,None; /bin/bash" 
```
...
```bash
docker run --rm -p 5901:5901 -it teamx-ros-gz bash -c "TVNC_WM=xfce /opt/TurboVNC/bin/vncserver; /bin/bash" 
```
...
```bash
docker run --rm -p 5901:5901 -v path/to/foler:where/it/goes/in/container -it teamx-ros-gz bash -c "TVNC_WM=xfce /opt/TurboVNC/bin/vncserver -securitytypes TLSNone,X509None,None; /bin/bash" 
```
